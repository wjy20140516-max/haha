...
#include "TB_Mem_Main_Read_vsInvCKB_DDW.h"
#include "Common.h"
...
* Run test
*/
void TB_Mem_Main_Read_vsInvCKB_DDW::Run(TestItemExecuteContext * aContext) {
	this->UserRun(aContext);
	//Add code here

    //this->Info("[Test Item] TB_Mem_Main_Read_vsInvCKB_DDW -- started");
    ...
    Print("TB_Mem_Main_Read_vsInvCKB_DDW- array ecr  read cost %.6f second", tm.count() / 1000);
    //testlog.Info("TB_Mem_Main_Read_vsInvCKB_DDW- array ecr  read cost {:.6f} second", tm.count() / 1000);
    update_local_time();
    // Data Check and Print
    auto start1 = std::chrono::high_resolution_clock::now();
    Data_Compare_CKB_DDW_array_print(raw_data_fmt, Read_Bytes, All_DUTs_Date, DDW_RESET_InvCKB_DATA, error_bfr, &error_flag, Mbist_Start_Page, TFB_Limit, DFB_Limit, aContext);
    auto end1 = std::chrono::high_resolution_clock::now();
    std::chrono::duration<double, std::milli> tm1 = end1 - start1;
    Print("TB_Mem_Main_Read_vsInvCKB_DDW- array print log cost %.6f second", tm1.count() / 1000);
    //testlog.Info("TB_Mem_Main_Read_vsInvCKB_DDW- array print log cost {:.6f} second", tm1.count() / 1000);
    // Judge
    DUT_LOOP_START
    if ((error_flag.error_flags[dut-1][DFB_LMT_FLAG] != 1) || (error_flag.error_flags[dut-1][TFB_LMT_FLAG] != 1))
    {
    (*item)->SetValue(1);
    TB_Mem_Main_Read_vsInvCKB_DDW_1270[dut-1] = 1;
    }
    else
    {
    (*item)->SetValue(0);
    TB_Mem_Main_Read_vsInvCKB_DDW_1270[dut - 1] = 0;
    }
    DUT_LOOP_END
    // Shmoo Stop
    ...
    //this->Info("[Test Item] TB_Mem_Main_Read_vsInvCKB_DDW -- end");
    }

/**
* Constructor
*/
TB_Mem_Main_Read_vsInvCKB_DDW::TB_Mem_Main_Read_vsInvCKB_DDW(TestItemExecuteContext * aContext) : TestItem("TB_Mem_Main_Read_vsInvCKB_DDW", aContext){
...
* Destructor
*/
TB_Mem_Main_Read_vsInvCKB_DDW::~TB_Mem_Main_Read_vsInvCKB_DDW() {
	//DO NOT add code here unless necessary.
}
...
* Init test
*/
bool TB_Mem_Main_Read_vsInvCKB_DDW::Initialize(TestItemExecuteContext * aContext) {
...
* Exit test
*/
void TB_Mem_Main_Read_vsInvCKB_DDW::Exit(TestItemExecuteContext * aContext) {
...
* Pause test
*/
void TB_Mem_Main_Read_vsInvCKB_DDW::Pause(TestItemExecuteContext * aContext) {
...
* Resume test
*/
void TB_Mem_Main_Read_vsInvCKB_DDW::Resume(TestItemExecuteContext * aContext) {
...
* Stop test
*/
void TB_Mem_Main_Read_vsInvCKB_DDW::Stop(TestItemExecuteContext * aContext) {
...
* AfterBin test
*/
void TB_Mem_Main_Read_vsInvCKB_DDW::AfterBin(TestItemExecuteContext * aContext) {
...
*/
REGISTER_REFLECTABLE_TYPE_WITH_PARAMETER2(TB_Mem_Main_Read_vsInvCKB_DDW, TestItemExecuteContext *)
