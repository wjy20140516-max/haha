...
#include "TB_Mbist_Write_InvCKB_DDW.h"
#include "Common.h"
...
* Run test
*/
void TB_Mbist_Write_InvCKB_DDW::Run(TestItemExecuteContext * aContext) {
    ...
    // ETLog::Info("Trigger");
    // sleep(20);
    MbistWriteDDW(Mbist_Start_Page, DDW_RESET_InvCKB_DATA);
    #ifdef POLLING_DELAY
    ...
    Print("- Issue MBIST WRITE: MEM_Address=%04X00h, MEM_Data=AAh, MBIST in DDW Mode, Cycle#%d, dut<%d:1>=0x%02X)", Mbist_Start_Page, cycle + 1, MAX_DUTS, MAX_DUTS);
    ...
    auto end = std::chrono::high_resolution_clock::now();
    std::chrono::duration<double, std::milli> tm = end - start;
    Print("TB_Mbist_Write_InvCKB_DDW- array ecr  read cost %.6f second", tm.count() / 1000);
    //testlog.Info("TB_Mbist_Write_InvCKB_DDW- array ecr  read cost {:.6f} second", tm.count() / 1000);
    update_local_time;
    //Data Check and Print
    auto start1 = std::chrono::high_resolution_clock::now();
    Data_Compare_CKB_DDW_array_print(raw_data_fmt, Read_Bytes, All_DUTs_Date, DDW_RESET_InvCKB_DATA, error_bfr, &error_flag, Mbist_Start_Page, TFB_Limit, DFB_Limit, aContext, false);
    auto end1 = std::chrono::high_resolution_clock::now();
    std::chrono::duration<double, std::milli> tm1 = end1 - start1;
    Print("TB_Mbist_Write_InvCKB_DDW- array print log cost %.6f second", tm1.count() / 1000);
    //testlog.Info("TB_Mbist_Write_InvCKB_DDW- array print log cost {:.6f} second", tm1.count() / 1000);
    //Judge
    DUT_LOOP_START
    ...
    {
    (*item)->SetValue(1);
    if (testitem == "1260_TB_Mbist_Write_InvCKB_DDW")
    {
    TB_Mbist_Write_InvCKB_DDW_1260[dut-1] = 1;
    }
    if (testitem == "1280_TB_Mbist_Write_InvCKB_DDW")
    {
    TB_Mbist_Write_InvCKB_DDW_1280[dut-1] = 1;
    }
    }
    else
    {
    ...
    if (testitem == "1260_TB_Mbist_Write_InvCKB_DDW")
    {
    TB_Mbist_Write_InvCKB_DDW_1260[dut - 1] = 0;
    }
    if (testitem == "1280_TB_Mbist_Write_InvCKB_DDW")
    {
    TB_Mbist_Write_InvCKB_DDW_1260[dut - 1] = 0;
    ...
    //this->Info("[Test Item] TB_Mbist_Write_InvCKB_DDW -- end");
    }

/**
* Constructor
*/
TB_Mbist_Write_InvCKB_DDW::TB_Mbist_Write_InvCKB_DDW(TestItemExecuteContext * aContext) : TestItem("TB_Mbist_Write_InvCKB_DDW", aContext){
...
* Destructor
*/
TB_Mbist_Write_InvCKB_DDW::~TB_Mbist_Write_InvCKB_DDW() {
	//DO NOT add code here unless necessary.
}
...
* Init test
*/
bool TB_Mbist_Write_InvCKB_DDW::Initialize(TestItemExecuteContext * aContext) {
...
* Exit test
*/
void TB_Mbist_Write_InvCKB_DDW::Exit(TestItemExecuteContext * aContext) {
...
* Pause test
*/
void TB_Mbist_Write_InvCKB_DDW::Pause(TestItemExecuteContext * aContext) {
...
* Resume test
*/
void TB_Mbist_Write_InvCKB_DDW::Resume(TestItemExecuteContext * aContext) {
...
* Stop test
*/
void TB_Mbist_Write_InvCKB_DDW::Stop(TestItemExecuteContext * aContext) {
...
* AfterBin test
*/
void TB_Mbist_Write_InvCKB_DDW::AfterBin(TestItemExecuteContext * aContext) {
...
*/
REGISTER_REFLECTABLE_TYPE_WITH_PARAMETER2(TB_Mbist_Write_InvCKB_DDW, TestItemExecuteContext *)
