...
#include "TB_Mem_Ext_Populate_vhhtrim.h"
#include "Common.h"
...
* Run test
*/
void TB_Mem_Ext_Populate_vhhtrim::Run(TestItemExecuteContext * aContext) {
	this->UserRun(aContext);
	//Add code here

    //this->Info("[Test Item] TB_Mem_Ext_Populate_vhhtrim -- started");
    // AC Param
    ...
    //    true
    //    );
    std::vector<unsigned char> page_array_cpy;
    page_array_cpy.reserve(size);
    for (auto val : page_array_cpy) {
    page_array_cpy.push_back(static_cast<unsigned char>(val & 0xFF));
    }
    // <RM 2025/07/18: updata vhh trim only>
    UserRegs.get_trim_val_update(dutid, 0, page_array_cpy);
    UserRegs.set_xreg_target(page_array_cpy);
    // Write to UBM
    //!!! unsigned char arrayUBM[size];
    for (int i = 0; i < size; i++)
    {
    //arrayUBM[i] = static_cast<unsigned char>(page_array_cpy1[i] & 0xFF);
    UbmData[dut - 1][i] = static_cast<int>(page_array_cpy[i]);
    }
    ...
    {
    (*item)->SetValue(1);
    TB_Mem_Ext_Populate_vhhtrim_1350[dut - 1] = 1;
    }
    DUT_LOOP_END
    }
    //this->Info("[Test Item] TB_Mem_Ext_Populate_vhhtrim -- end");
    }

/**
* Constructor
*/
TB_Mem_Ext_Populate_vhhtrim::TB_Mem_Ext_Populate_vhhtrim(TestItemExecuteContext * aContext) : TestItem("TB_Mem_Ext_Populate_vhhtrim", aContext){
...
* Destructor
*/
TB_Mem_Ext_Populate_vhhtrim::~TB_Mem_Ext_Populate_vhhtrim() {
	//DO NOT add code here unless necessary.
}
...
* Init test
*/
bool TB_Mem_Ext_Populate_vhhtrim::Initialize(TestItemExecuteContext * aContext) {
...
* Exit test
*/
void TB_Mem_Ext_Populate_vhhtrim::Exit(TestItemExecuteContext * aContext) {
...
* Pause test
*/
void TB_Mem_Ext_Populate_vhhtrim::Pause(TestItemExecuteContext * aContext) {
...
* Resume test
*/
void TB_Mem_Ext_Populate_vhhtrim::Resume(TestItemExecuteContext * aContext) {
...
* Stop test
*/
void TB_Mem_Ext_Populate_vhhtrim::Stop(TestItemExecuteContext * aContext) {
...
* AfterBin test
*/
void TB_Mem_Ext_Populate_vhhtrim::AfterBin(TestItemExecuteContext * aContext) {
...
*/
REGISTER_REFLECTABLE_TYPE_WITH_PARAMETER2(TB_Mem_Ext_Populate_vhhtrim, TestItemExecuteContext *)
