...
#include "TB_Read_SecurityRegisters.h"
#include "Common.h"
...
* Run test
*/
void TB_Read_SecurityRegisters::Run(TestItemExecuteContext * aContext) {
	this->UserRun(aContext);
	//Add code here

    //this->Info("[Test Item] TB_Read_SecurityRegisters -- started");
    int Page_addr;
    string pa;
    int Page_Length;
    ...
    Digital.Pattern.SetRegister(ONES_REG_ZK, Page_addr);
    Digital.Pattern.RunRegister("pReadSecurityRegisters", ENUM_PATTERNSTOP_INTERRUPT, 5 S);
    Digital.Pattern.RunModule("pReadSecurityRegisters", "fReadSecurityRegisters", ENUM_PATTERNSTOP_INTERRUPT, 5 S);
    Time.Delay(1 MS);
    ...
    {
    Print("[SecurityReg]DUT%d Reg 5Ah = %02X %02X %02X %02X %02X %02X %02X %02X %02X %02X %02X %02X %02X %02X %02X %02X",
    ...
    //testlog.Info("[SecurityReg]DUT{} Reg 48h = %02X %02X %02X %02X %02X %02X %02X %02X %02X %02X %02X %02X %02X %02X %02X %02X",
    //    itDUT->GetSysID(), Data[r], Data[r + 1], Data[r + 2], Data[r + 3], Data[r + 4], Data[r + 5], Data[r + 6], Data[r + 7],
    //    Data[r + 8], Data[r + 9], Data[r + 10], Data[r + 11], Data[r + 12], Data[r + 13], Data[r + 14], Data[r + 15]);
    r = r + 16;
    }
    DUT_LOOP_END
    //this->Info("[Test Item] TB_Read_SecurityRegisters -- end");
    }

/**
* Constructor
*/
TB_Read_SecurityRegisters::TB_Read_SecurityRegisters(TestItemExecuteContext * aContext) : TestItem("TB_Read_SecurityRegisters", aContext){
...
* Destructor
*/
TB_Read_SecurityRegisters::~TB_Read_SecurityRegisters() {
	//DO NOT add code here unless necessary.
}
...
* Init test
*/
bool TB_Read_SecurityRegisters::Initialize(TestItemExecuteContext * aContext) {
...
* Exit test
*/
void TB_Read_SecurityRegisters::Exit(TestItemExecuteContext * aContext) {
...
* Pause test
*/
void TB_Read_SecurityRegisters::Pause(TestItemExecuteContext * aContext) {
...
* Resume test
*/
void TB_Read_SecurityRegisters::Resume(TestItemExecuteContext * aContext) {
...
* Stop test
*/
void TB_Read_SecurityRegisters::Stop(TestItemExecuteContext * aContext) {
...
* AfterBin test
*/
void TB_Read_SecurityRegisters::AfterBin(TestItemExecuteContext * aContext) {
...
*/
REGISTER_REFLECTABLE_TYPE_WITH_PARAMETER2(TB_Read_SecurityRegisters, TestItemExecuteContext *)
